...
func NewEventLoop(opts ...Option) *EventLoop {
	...
	loop.vm.Set("setImmediate", loop.setImmediate)
	loop.vm.Set("clearTimeout", loop.clearTimeout)
	loop.vm.Set("clearInterval", loop.clearInterval)
	loop.vm.Set("clearImmediate", loop.clearImmediate)

	// loop.vm.SetMaxCallStackSize(2 ^ 16)
	// loop.vm.SetAsyncContextTracker(newAsyncTracker(loop.eventChan))
	// loop.vm.SetPromiseRejectionTracker(func(p *goja.Promise, operation goja.PromiseRejectionOperation) {
	// 	if operation == goja.PromiseRejectionHandle {
	// 		loop.eventChan <- "handled"
	// 		return
	// 	}
	// 	loop.eventChan <- "rejected"
	// })
...
func (loop *EventLoop) Registry() *require.Registry {
...
// If the loop is already started it will panic.
func (loop *EventLoop) Start() *goja.Runtime {
	loop.setRunning()
	go loop.run(true)
	return loop.vm
}

// Runtime returns the goja.Runtime instance associated with the loop.
